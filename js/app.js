(async function(){
  ...
  function seedState(){
    ...
    state.modules.push(
      {id:id(),ownerType:'profile',ownerId:ddsProfile.id,moduleType:'recipe',title:'Drugs',recipes:[
        ...
        {id:id(),name:'Meth',base:'Meth',quantity:'8 bags',notes:'Needs more cut',ingredients:[
          {id:id(),name:'Meth',percent:80,notes:''}
        ]}
      ]},
  ...
  function renderHome(){
    ...
    el.view.innerHTML = `
      <section class="panel">
        <div class="hero">
          <div class="panel-title">
            ...
            <div class="muted">Now using IndexedDB with migration from older saves.</div>
          </div>
      ...
      <section class="kpi-grid">
        ...
        <div class="kpi"><div class="muted">Favorites</div><div class="kpi-value">${activeGames.filter(g => g.favorite).length}</div></div>
      </section>
      <section class="panel">
  ...
  function openModuleForm(ownerType, ownerId, m=null){
    ...
    if (m){
      ...
      gid('saveAsPresetBtn').onclick = () => {
        if (m.moduleType === 'fields') state.presets.push({ id:id(), name:m.title, moduleType:'fields', fields:structuredClone(m.fields) });
        if (m.moduleType === 'table') state.presets.push({ id:id(), name:m.title, moduleType:'table', columns:structuredClone(m.columns) });
        if (m.moduleType === 'recipe') state.presets.push({ id:id(), name:m.title, moduleType:'recipe', recipes:[] });
        if (m.moduleType === 'chain') state.presets.push({ id:id(), name:m.title, moduleType:'chain', chains:[] });
        persist(); closeModal(); render();
      };
    }
  }
  ...
  function openPresetPicker(ownerType, ownerId){
    ...
    document.querySelectorAll('[data-use-preset]').forEach(b => b.onclick = () => {
      const p = state.presets.find(x => x.id === b.dataset.usePreset); if (!p) return;
      const fresh = { id:id(), ownerType, ownerId, moduleType:p.moduleType, title:p.name };
      if (p.moduleType === 'fields') fresh.fields = structuredClone(p.fields);
      if (p.moduleType === 'table') { fresh.columns = structuredClone(p.columns); fresh.rows = []; }
      if (p.moduleType === 'recipe') fresh.recipes = [];
      if (p.moduleType === 'chain') fresh.chains = [];
      state.modules.push(fresh); persist(); closeModal(); render();
    });
  }

  function openPresetEditor(pid){
    const p = state.presets.find(x => x.id === pid); if (!p) return;
    openModal(`<div class="modal-shell"><div class="modal-head"><h3>Edit Preset</h3></div><div class="modal-body"><label><div class="small-note">Name</div><input id="presetNameInput" value="${esc(p.name)}" /></label><div class="modal-actions"><span></span><div class="modal-right-actions"><button id="closeModalBtn" class="ghost-btn">Cancel</button><button id="savePresetBtn" class="gold-btn">Save</button></div></div></div></div>`);
    gid('closeModalBtn').onclick = closeModal;
    gid('savePresetBtn').onclick = () => { p.name = gid('presetNameInput').value.trim() || p.name; persist(); closeModal(); render(); };
  }
  ...
  function seedServerModules(server){
    state.modules.push(
      {id:id(),ownerType:'server',ownerId:server.id,moduleType:'fields',title:'Server Overview',fields:[
        {key:'version',label:'Version',type:'text',value:''},
        {key:'platform',label:'Platform',type:'text',value:''}
      ]}
    );
  }
})().catch(err => {
